...
shape_var_list = ['probe_sieie', "probe_hoe",'probe_ecalPFClusterIso', 'probe_trkSumPtHollowConeDR03', 'probe_hcalPFClusterIso', 'probe_pfChargedIso', 'probe_phiWidth', 'probe_trkSumPtSolidConeDR04', 'probe_r9', 'probe_pfChargedIsoWorstVtx', 'probe_s4', 'probe_etaWidth', 'probe_mvaID', 'probe_sieip']
legend_loc=["upper left","upper right","upper right","upper right", "upper right", "upper right", "upper right", "upper right", "upper left", "upper right", "upper left", "upper right", "upper left", "upper left"]
...
input_mc = pd.read_parquet(f"{args.inp}/MC_{args.dataset}_{args.detector}.parquet", engine="pyarrow")
input_data = pd.read_parquet(f"{args.inp}/Data_{args.dataset}_{args.detector}.parquet", engine="pyarrow")
input_data["weight"]=1
input_mc["weight"]*=(input_data["weight"].sum()/input_mc["weight"].sum())

for q in input_mc.columns:
    print(q)
...
for idx,v in enumerate(shape_var_list):
    ...
    if v == "probe_mvaID":
        lo, hi = (-0.9,1.0)
    elif v in ['probe_ecalPFClusterIso','probe_trkSumPtHollowConeDR03',"probe_hoe",'probe_hcalPFClusterIso','probe_pfChargedIso','probe_trkSumPtSolidConeDR04','probe_pfChargedIsoWorstVtx']:
    ...
    hists['mc'] = np.histogram(input_mc[v], nbins, (lo,hi), weights=input_mc['weight'])
    hists['mc_sumw2'] = np.histogram(input_mc[v], nbins, (lo,hi), weights=input_mc['weight']**2)
   ...
   # chi2_flow = np.sum( (hists["data"][0]-hists["mc_flow"][0])**2/(hists["mc_flow_sumw2"][0]+hists["data"][0]) ) / (len(hists["data"][1])-1)
    ...
    #     w2method = "poisson",
    #     ax = ax[0],
    #     label = f"MC, rwgt-corrected $\chi^2$/d.o.f.={chi2_rwgt:.1f} ",
    #     histtype = "errorbar",
    #     flow = "none",
    #     color = '#e42536'
    # )
    ...
    #     w2method = "poisson",
    #     ax = ax[0],
    #     label = f"MC, flow-corrected $\chi^2$/d.o.f.={chi2_flow:.1f}",
    #     histtype = "errorbar",
    #     flow = "none",
    #     color = '#f89c20'
    # )
    ...
    if v in ['probe_ecalPFClusterIso','probe_trkSumPtHollowConeDR03','probe_hcalPFClusterIso','probe_pfChargedIso','probe_trkSumPtSolidConeDR04','probe_pfChargedIsoWorstVtx',"probe_hoe"]:
        ax[0].set_yscale("log")
