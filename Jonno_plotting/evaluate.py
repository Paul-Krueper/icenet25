...
print("norm changed by S2_unnorm / S2 = " +str(events['w_post_S2_unnorm'].sum()/events['w_post_S2'].sum()))
print(" S2_unnorm / S1 = " +str(events['w_post_S2_unnorm'].sum()/events['w_post_S1'].sum()))
print("mean S2_unnorm / S1 "+ str(np.average(events['w_post_S2_unnorm']/events['w_post_S1'],weights=events["w_post_S1"])))
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
events.to_parquet(f"{args.out}/{name}", engine="pyarrow")

events_data['y'] = 1
name = re.sub(".parquet", "_processed.parquet", args.input_data.split("/")[-1])
events_data.to_parquet(f"{args.out}/{name}", engine="pyarrow")
